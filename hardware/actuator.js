...
const { cylinder, cylinderElliptic, polygon, roundedRectangle } = jscad.primitives
...
function actuator() {
  ...
  return subtract(
    hex(0, rightX, 0),
    // motor hole
    axialCylinder(11, leftX, rightX, 80 * qty),
    // pulley hole
    axialCylinder(19, sh, rightX, 120 * qty),
    switchhole(),
    // tophole
    cylinder({radius: 1.4, center: [centerX, holeY, topZ], height: 10, segments: 30 * qty}),
    screws()
  )
}
...
function screws() {
  const screw = rotateY(Math.PI / 2, [
    cylinder({radius: 1.6, center: [0, 0, 1.5], height: 3, segments: 30 * qty}),
    cylinderElliptic({startRadius: [0.01, 0.01], endRadius: [3, 3], center: [0, 0, 0.5], height: 3.5, segments: 30 * qty})
  ])
  return [
...
function switchhole() {
  return translate([sh, 0, topZ - 13], rotateY(Math.PI / 2,
    extrudeLinear({height: sizeX}, roundedRectangle({size: [20, 20], roundRadius: 2.5, segments: 40 * qty}))
  ))
}
...
function motor() {
  return [
    ...
    axialCylinder(motorRadius, -85, -29, 80 * qty), // motor
    axialCylinder(motorRadius, -28, 0, 80 * qty), // gear box
...
function spool() {
  return [
    axialCylinder(8, 2, 15, 80 * qty), // coupling
    axialCylinder(pulleyRadius, 2, 4, 140 * qty), // spool inside
    axialCylinder(pulleyRadius, 9, 11, 140 * qty) // spool outside
  ]
}
